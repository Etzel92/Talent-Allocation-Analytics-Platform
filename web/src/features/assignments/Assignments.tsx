...
function toCSV(rows: Row[]) {
  ...
  const body = rows.map((r) => [r.id, r.city, r.education, r.experience, r.payment_tier].join(','));
...
export default function Assignments() {
  ...
  if (expMin !== undefined)
    activeChips.push({ label: `Exp ≥ ${expMin}`, onDelete: () => setExpMin(undefined) });
  if (expMax !== undefined)
    activeChips.push({ label: `Exp ≤ ${expMax}`, onDelete: () => setExpMax(undefined) });
  if (city) activeChips.push({ label: `City: ${city}`, onDelete: () => setCity('') });
  if (education)
    activeChips.push({ label: `Education: ${education}`, onDelete: () => setEducation('') });
  if (skills.length)
    activeChips.push({ label: `Skills: ${skills.join(' · ')}`, onDelete: () => setSkills([]) });
  ...
  const cols = React.useMemo<GridColDef<Row>[]>(
    () => [
      ...
      { field: 'experience', headerName: 'Experience', type: 'number', flex: 0.8, minWidth: 120 },
      { field: 'payment_tier', headerName: 'Tier', type: 'number', flex: 0.6, minWidth: 90 },
    ],
    [],
  );

 
  const [colVis, setColVis] = React.useState<GridColumnVisibilityModel>({
    education: isMdUp,
  });
  React.useEffect(() => {
    setColVis((prev) => ({ ...prev, education: isMdUp }));
  }, [isMdUp]);
  ...
  return (
    <Container
      maxWidth="xl"
      disableGutters
      sx={{ px: { xs: 1, sm: 2, md: 3 }, py: { xs: 2, md: 3 } }}
    >
      <Stack
      ...
      >
        ...
        <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap' }}>
          <Button
            ...
            onClick={exportCsv}
            disabled={!didSearch || !rows.length}
          >
            Export to CSV
          </Button>
        </Stack>
      ...
      <Paper sx={{ p: { xs: 1.5, md: 2 }, mb: 2 }}>
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField {...params} label="Skills / Domain" sx={{ mt: 2 }} />
          )}
        />
      ...
      {!didSearch ? (
        ...
        <Paper sx={{ p: { xs: 1, md: 1.5 }, overflowX: 'auto' }}>
